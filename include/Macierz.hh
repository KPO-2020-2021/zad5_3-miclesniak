 ...
 */
  ...
  /**
   * @brief Konstruktor paramtetryczny macierzy
   * 
   * @param tmp tablica z której której przepisujemy wartości do Macierzy
   */
  Macierz(double tmp[Wymiar][Wymiar])
  {
    for (int i = 0; i < Wymiar; i++)
    {
      for (int j = 0; j < Wymiar; j++)
  ...
  /**
   * @brief Funkcja porównania dwóch Macierzy
   * 
   * @param tmp Macierz z którą porównujemy
   * @return true Macierze są identyczne    
   * @return false Macierze różnią się od siebie
   */
  bool operator==(Macierz<Wymiar> const tmp) const
  {
    for (int i = 0; i < Wymiar; ++i)
    {
      for (int j = 0; j < Wymiar; ++j)
